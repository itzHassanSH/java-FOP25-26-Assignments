...
public record Channel(
    ...
    public boolean remove(String title) {
        // TODO: H12.1.1
        Optional<Video> videoToRemove = videos.stream().filter(video -> Objects.equals(video.title(), title)).findFirst();

        // Here the map only carries out v -> videos.remove(v) IF a value exists in Optional
        // This is why .orElse(false) works together
        return videoToRemove.map(videos::remove).orElse(false);
    }
    ...
    public Set<User> getSubscribersBetween(LocalDateTime start, LocalDateTime end) {
        // TODO: H12.1.2
        return subscribers.entrySet().stream().filter(o -> o.getValue().isAfter(start) && o.getValue().isBefore(end)).map(Map.Entry::getKey).collect(Collectors.toSet());
    }
    ...
    public Set<User> getLatestSubscribers(Month month, int count) {
        // TODO: H12.1.3
        return subscribers.entrySet().stream().filter(o -> o.getValue().getMonth() == month).map(Map.Entry::getKey).limit(count).collect(Collectors.toSet());
    }
}
