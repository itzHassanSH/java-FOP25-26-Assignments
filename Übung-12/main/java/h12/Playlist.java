...
public record Playlist(
    ...
    public Duration getTotalWatchTime() {
        // TODO: H12.2.1
        return videos.stream().map(Video::duration).reduce(Duration::plus).orElse(Duration.ZERO);
    }
    ...
    public Stream<Video> generateRandomVideoPicker() {
        // TODO: H12.2.4
        // Bound may never be 0; slightly counter-intuitive but it doesn't cover for empty list
        return videos.isEmpty()? Stream.empty() : Stream.generate(() -> videos().get(randomizer.nextInt(videos().size())));
    }
    ...
    public int getNumberOfPublicVideos() {
        // TODO: H12.2.2
        return (int) videos.stream().filter(Video::isPublic).count();
    }
    ...
    public Map<User, Set<Video>> groupVideosByUser(Duration duration) {
        // TODO: H12.2.3
        return videos.stream().filter(v -> v.duration().compareTo(duration) >= 0).collect(Collectors.groupingBy(Video::uploader,Collectors.toSet()));
    }
}
