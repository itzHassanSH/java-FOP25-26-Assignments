...
public class Packet {
    ...
    public Packet(
        ...
        assert data == null || data.length() <= 8 && type == PacketType.DATA;
        this.data = data;
        this.header = new Header(sourcePort, destinationPort, sequenceNumber, calculateChecksum(sequenceNumber, data), type);
    }
    ...
    public void expectType(PacketType type) throws PacketTypeException {
        if (type != getHeader().type()) {
            throw new PacketTypeException(type, getHeader().type());
        }

        // TODO: H9.3 - remove if implemented
    }
    ...
    public void expectSequenceNumber(int seq) throws PacketSequenceException {
        if (seq != getHeader().sequenceNumber()) {
            throw new PacketSequenceException(getHeader().sequenceNumber(), seq);
        }
        // TODO: H9.3 - remove if implemented
    }
    ...
    public void validateChecksum() throws PacketChecksumException {
        int calculatedChecksum = calculateChecksum(header.sequenceNumber(), data);
        if (getHeader().checksum() != calculatedChecksum) {
            throw new PacketChecksumException(calculatedChecksum, getHeader().checksum());
        }
        // TODO: H9.3 - remove if implemented
    }
